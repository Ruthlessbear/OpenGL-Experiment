...
uniform vec2 clip_scale = vec2(0.5f, 0.5f);
uniform vec2 threshold_factor = vec2(0.13f, 0.13f);
...
void main()
{    
    float ss = fract(TexCoords.x * clip_scale.x);
    float tt = fract(TexCoords.y * clip_scale.y);

    if(ss > threshold_factor.x && tt > threshold_factor.y)
        discard;
